...
set(_exported_dependencies "rclcpp;rclcpp_action;rclcpp_lifecycle;plansys2_pddl_parser;ament_index_cpp;plansys2_core;plansys2_msgs;plansys2_domain_expert;std_msgs;lifecycle_msgs")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to plansys2_problem_expert_DEFINITIONS, plansys2_problem_expert_INCLUDE_DIRS,
# plansys2_problem_expert_LIBRARIES, and plansys2_problem_expert_LINK_FLAGS.
# Additionally collect the direct dependency names in
# plansys2_problem_expert_DEPENDENCIES as well as the recursive dependency names
# in plansys2_problem_expert_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(plansys2_problem_expert_DEPENDENCIES ${_exported_dependencies})
  set(plansys2_problem_expert_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(plansys2_problem_expert_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(plansys2_problem_expert_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(plansys2_problem_expert_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(plansys2_problem_expert_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(plansys2_problem_expert_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND plansys2_problem_expert_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
